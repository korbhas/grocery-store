        ...
        <div className="flex flex-col gap-3">
          ...
          {outOfStock ? (
            ...
            <div className="mt-2 flex items-center gap-4">
              <div className="flex items-center overflow-hidden rounded-lg border border-[#e23744]">
                ...
                <Button variant="ghost" size="icon" aria-label="Increase quantity" className="h-10 w-10 rounded-none" onClick={() => setQuantity(Math.min(product.stock_qty, quantity + 1))}>
                  <Plus size={16} />
                </Button>
              </div>
              {inCart ? (
                ...
                <Button className="h-10 rounded-lg bg-[#e23744] px-6 font-bold hover:bg-[#c52d39]" onClick={() => addToCart(product, quantity)}>
          ...
          <div className="mt-4 rounded-xl border bg-card p-4 text-sm">
            <div className="flex justify-between border-b pb-2">
              ...
              <span className={`font-medium ${product.stock_qty > 0 ? 'text-green-600' : 'text-destructive'}`}>
